...
@section('title', 'Edit Jalur - Admin Mountix')
...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-black text-neutral-dark">Edit Jalur: {{ $jalur->nama_jalur }}</h1>
    <p class="text-neutral-dark/60">Perbarui informasi rute pendakian untuk gunung {{ $jalur->gunung->nama }}.</p>
</div>
    ...
    <form action="{{ route('admin.jalur.update', $jalur->id) }}" method="POST">
        ...
        <div class="bg-white p-8 rounded-3xl border border-neutral-light shadow-sm space-y-6">
            <div>
                ...
                <select name="gunung_id" required class="w-full px-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all appearance-none">
                    @foreach($gunungs as $g)
                        <option value="{{ $g->id }}" {{ $jalur->gunung_id == $g->id ? 'selected' : '' }}>{{ $g->nama }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nama_jalur" value="{{ old('nama_jalur', $jalur->nama_jalur) }}" required class="w-full px-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all">
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" class="w-full px-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all">{{ old('deskripsi', $jalur->deskripsi) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga_per_orang" value="{{ old('harga_per_orang', $jalur->harga_per_orang) }}" required class="w-full pl-10 pr-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all">
                    </div>
                ...
                <div>
                    ...
                    <input type="number" name="kuota_default" value="{{ old('kuota_default', $jalur->kuota_default) }}" required class="w-full px-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="estimasi_jam" value="{{ old('estimasi_jam', $jalur->estimasi_jam) }}" required class="w-full pr-12 px-4 py-3 bg-neutral-light border border-transparent rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none transition-all">
