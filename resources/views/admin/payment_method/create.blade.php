...
@section('title', 'Tambah Metode Pembayaran - Admin Mountix')
...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-black text-neutral-dark">Tambah Metode</h1>
</div>

<div class="max-w-4xl">
    <form action="{{ route('admin.payment-method.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div class="bg-white p-8 rounded-3xl border border-neutral-light shadow-sm space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: BCA" class="w-full px-4 py-3 bg-neutral-light/30 border border-neutral-light rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium" required>
                </div>
            </div>

            <!-- Bank Fields -->
            <div id="bank-fields" class="{{ old('type', 'bank') === 'bank' ? '' : 'hidden' }} grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label class="text-xs font-bold uppercase tracking-wider text-neutral-dark/60">Nomor Rekening</label>
                    <input type="text" name="account_number" value="{{ old('account_number') }}" placeholder="000-000-000" class="w-full px-4 py-3 bg-neutral-light/30 border border-neutral-light rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium">
                </div>
                <div class="space-y-2">
                    <label class="text-xs font-bold uppercase tracking-wider text-neutral-dark/60">Atas Nama</label>
                    <input type="text" name="account_name" value="{{ old('account_name') }}" placeholder="NAMA PEMILIK" class="w-full px-4 py-3 bg-neutral-light/30 border border-neutral-light rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium">
                </div>
            </div>

            <!-- QRIS Fields -->
            <div id="qris-fields" class="{{ old('type') === 'qris' ? '' : 'hidden' }} space-y-2">
                <label class="text-xs font-bold uppercase tracking-wider text-neutral-dark/60">Upload QR Code</label>
                ...
                <p class="text-[10px] text-neutral-dark/40 italic">*Format: JPG, PNG. Max 2MB.</p>
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', '1') ? 'checked' : '' }} class="w-5 h-5 rounded-lg accent-primary">
